...
JPM$Data <- as.Date(JPM$Data, format = "%d.%m.%Y")
...
# Calculate beta for JPM against S&P 500 using excess returns
JPM_with_sp500 <- JPM %>% 
  select(Data, ExcessReturn) %>%
  rename(JPM_ExcessReturn = ExcessReturn) %>%
  inner_join(sp500 %>% select(Data, ExcessReturn) %>% rename(SP500_ExcessReturn = ExcessReturn), by = "Data")

beta_fit <- lm(JPM_ExcessReturn ~ SP500_ExcessReturn, data = JPM_with_sp500)
# Calculate the beta value
beta_value <- round(beta_fit$coefficients["SP500_ExcessReturn"], 2)

# Calculate beta for JPM against S&P 500 using excess returns for the new period
JPM_with_sp500_2008 <- JPM2008 %>%
  select(Data, ExcessReturn) %>%
  rename(JPM_ExcessReturn = ExcessReturn) %>%
...
beta_fit_2008 <- lm(JPM_ExcessReturn ~ SP500_ExcessReturn, data = JPM_with_sp500_2008)
beta_2008 <- round(beta_fit_2008$coefficients["SP500_ExcessReturn"], 2)
alpha_2008 <- round(beta_fit_2008$coefficients["(Intercept)"], 2)


# Calculate beta for JPM against S&P 500 using excess returns for the first date range (2008-06-16 to 2019-12-12)
JPM_with_sp500_2008_to_2019 <- JPM_2008_to_2019 %>%
  select(Data, ExcessReturn) %>%
  rename(JPM_ExcessReturn = ExcessReturn) %>%
...
beta_fit_2008_to_2019 <- lm(JPM_ExcessReturn ~ SP500_ExcessReturn, data = JPM_with_sp500_2008_to_2019)
...
# Calculate beta for JPM against S&P 500 using excess returns for the second date range (2019-12-13 to 2023-09-01)
JPM_with_sp500_2019_to_2023 <- JPM_2019_to_2023 %>%
  select(Data, ExcessReturn) %>%
  rename(JPM_ExcessReturn = ExcessReturn) %>%
...
beta_fit_2019_to_2023 <- lm(JPM_ExcessReturn ~ SP500_ExcessReturn, data = JPM_with_sp500_2019_to_2023)
...
final_plot <- ggplot(excess_data, aes(x = Data, y = CumulativeExcessReturn, color = data)) +
  ...
  labs(
    title = "Cumulative Excess Return of JPM and S&P 500",
    subtitle = paste("Beta of JPM (against S&P 500 Excess Returns):", beta_value),
    x = "Date",
