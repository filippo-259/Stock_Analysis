...
return_jpm <- as.numeric(gsub(",", ".", gsub("%", "", jpm$Var.)))
...
# Calculate excess returns for jpm and S&P 500
excess_returns_jpm <- calc_excess_return(return_jpm, tbill$Price)
...
# Print excess returns and descriptive statistics for jpm and S&P 500
print(head(excess_returns_jpm, 10))
cat("\nDescriptive Statistics for jpm:")
desc_stats(excess_returns_jpm)
...
# Calculate and print alpha and beta for jpm compared to S&P 500
model_jpm_sp500 <- calc_alpha_beta(excess_returns_jpm, excess_returns_sp500)
cat("\nModel Summary (jpm vs S&P 500):\n")
print(summary(model_jpm_sp500))

# Combine data into one dataframe for ggplot
combined_data <- na.omit(data.frame(
  Market_Excess_Return = excess_returns_sp500, 
  JPM_Excess_Return = excess_returns_jpm
))

# Create an aesthetically appealing scatter plot with a regression line
ggplot(combined_data, aes(x = Market_Excess_Return, y = JPM_Excess_Return)) +
  geom_point(color = "darkorange", size = 1) +
  geom_smooth(method = "lm", color = "dodgerblue", size = 1) +
  labs(title = "Relationship between Excess Returns of jpm and S&P 500",
